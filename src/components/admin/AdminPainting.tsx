...
const PaintingCard = ({ order, colorFilter, onUpdatePainted }: {
  ...
  for (const [, positions] of colorsToShow) {
    for (const pos of positions) {
      sumQty += pos.qty;
      sumPainted += Math.min(painted[pos.posKey] || 0, pos.qty);
    }
  }
            ...
            <div className="flex flex-wrap gap-2 text-[11px]">
              ...
              {order.due_painting && (
                <span className="px-2 py-0.5 rounded-full bg-primary/8 text-primary/80 border border-primary/20">
                  Покраска до: {fmtDateShort(order.due_painting)}
                </span>
              )}
            </div>
            {/* Цветовые плашки */}
          ...
          {colorsToShow.map(([color, positions]) => {
            ...
            return (
              <div key={color} className="border-b border-primary/10 last:border-0">
                {/* Заголовок цвета */}
                <div
                  className="px-4 py-2 flex items-center gap-2 font-bold text-sm"
      ...
      <div className="flex flex-wrap items-center gap-4 mb-5">
        ...
        <div className="flex gap-2">
          {(['all', 'ready'] as const).map(mode => (
            ...
            >
              {mode === 'all' ? 'Все заказы' : 'Сплетены 100%'}
            </button>
          ))}
        </div>
      </div>
